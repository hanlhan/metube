...
class DownloadQueue:
    ...
    def __calc_download_path(self, quality, format, folder, playlist_title):
    ...
    async def __add_entry(self, entry, quality, format, folder, custom_name_prefix, already, playlist_title=''):
        ...
        if etype == 'playlist':
            ...
            for index, etr in enumerate(entries, start=1):
                ...
                results.append(await self.__add_entry(etr, quality, format, folder, custom_name_prefix, already, playlist_title))
            ...
            if not self.queue.exists(entry['id']):
                ...
                dldirectory, error_message = self.__calc_download_path(quality, format, folder, playlist_title)
            ...
            return await self.add(entry['url'], quality, format, folder, custom_name_prefix, already, playlist_title)
        return {'status': 'error', 'msg': f'Unsupported resource "{etype}"'}

    async def add(self, url, quality, format, folder, custom_name_prefix, already=None, playlist_title=''):
        ...
        return await self.__add_entry(entry, quality, format, folder, custom_name_prefix, already, playlist_title)
